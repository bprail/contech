...
SimpleCacheBackend::SimpleCacheBackend(uint64_t c, uint64_t s, int printMissLoc) {
    ...
    //contextCacheState[0] = sc;
    
    sharedCache = new SimpleCache(c, s);
...
void SimpleCacheBackend::updateBackend(MemReqContainer& mrc)
{
    ContextId ctid = mrc.ctid;
    bool rw = false;
    int32_t printCode = -1;
    
    /*if (dbg_count[(unsigned int)ctid] < 2)
    {
        //printf("%d %d (%d == %d)\n", ctid, dbg_count[(unsigned int)ctid], mrc.mav.size(), mrc.pushedOps);
        printCode = (unsigned int)ctid;
        dbg_count[(unsigned int)ctid]++;
    }*/
    ...
    for (auto iReq = mrc.mav.begin(), eReq = mrc.mav.end(); iReq != eReq; ++iReq, memOpPos++)
    {
        ...
        if (ma.type == action_type_memcpy)
        {
            ...
            do {
                ...
                if (srcAddress != 0)
                {
                    sharedCache->updateCache(false, accessSize, srcAddress, p_stats);
                    srcAddress += accessSize;
                    p_stats->accesses ++;
                }
                
                sharedCache->updateCache(true, accessSize, dstAddress, p_stats);
        ...
        do {
            ...
            if (!sharedCache->updateCache(rw, accessBytes, address, p_stats))
            {
                if (printMissLines == true)
                {
            ...
            if (printCode >= 0)
            {
                ...
                uint64_t tag = address >> ((global_c - global_s));
                cacheIdx &= ((0x1 << (global_c - (global_b + global_s))) - 1);
                printf("%d: %llx:%llx:%llx %d %c\n", printCode, tag, cacheIdx, offset,
                                                     accessBytes, hit);
            }*/
...
void SimpleCacheBackend::completeBackend(FILE* f, contech::TaskGraphInfo* tgi)
{
    ...
    double median = sharedCache->getMissRate();
    missRates.push_back(median);
    sum = median;
