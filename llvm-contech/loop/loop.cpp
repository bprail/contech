...
namespace llvm{
	...
	bool loop::isLoopControlIV(Loop *L, Instruction *IV) 
	{
	...
	const SCEVConstant *loop::getIncrmentFactorSCEV(ScalarEvolution *SE,
		const SCEV *SCEVExpr,
		Instruction &IV) 
	{
	...
	void loop::collectPossibleIVs(Loop *L) 
	{
		int cnt = 0;
		PossibleIVs.clear();
		NonLinearIvs.clear();
	...
	void loop::collectDerivedIVs(Loop *L, SmallInstructionVector IVs, SmallInstructionVector *DerivedIvs)
	{
		DerivedIvs->clear();
	...
	int loop::collectPossibleMemoryOps(GetElementPtrInst* gepAddr, SmallInstructionVector IVs, bool is_derived)
	{
		int cnt_elided = 0;
	...
	//bool loop::runOnLoop(Loop *L, LPPassManager &LPM) {
	bool loop::runOnFunction (Function &F) {
	...
	void loop::getAnalysisUsage(AnalysisUsage& AU) const {
		AU.setPreservesAll();
		AU.addRequired<ScalarEvolutionWrapperPass>();
		AU.addRequired<LoopInfoWrapperPass>();  //in this order
	}

	SmallInstructionVector loop:: getLoopMemoryOps() {
		outs() << "cnt_GetElementPtrInst\t" << cnt_GetElementPtrInst << "\n";
		outs() << "cnt_elided\t" << cnt_elided << "\n";
...
char loop::ID = 0;
RegisterPass<loop> X1("loop", "15745 Project");
}